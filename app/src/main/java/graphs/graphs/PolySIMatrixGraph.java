...
public class PolySIMatrixGraph<T> implements MutableGraph<T> {
  @Getter
  private final ImmutableBiMap<T, Integer> nodeMap; // a map mapping nodes to node indexes
  private final RoaringBitmap adjacency[];
  // private final long adjacency[][];
  // private static final int LONG_BITS = 64;

  public PolySIMatrixGraph(Graph<T> graph) {
    var topoOrder = topoLogicalSort(graph);
    var toNodeMap = ((Function<Collection<T>, ImmutableBiMap<T, Integer>>) nodes -> Streams
        .mapWithIndex(nodes.stream(), (n, idx) -> Pair.of(n, (int) idx))
  ...
  public PolySIMatrixGraph(Graph<T> graph, ImmutableBiMap<T, Integer> nodeMap) {
  ...
  private PolySIMatrixGraph(ImmutableBiMap<T, Integer> nodes) {
    nodeMap = nodes;
  ...
  public static <T> PolySIMatrixGraph<T> ofNodes(PolySIMatrixGraph<T> graph) {
    return new PolySIMatrixGraph<>(graph.nodeMap);
  }
  ...
  // private PolySIMatrixGraph(PolySIMatrixGraph<T> graph) {
  ...
  private PolySIMatrixGraph<T> bfsWithNoCycle(List<Integer> topoOrder) {
    var result = new PolySIMatrixGraph<T>(nodeMap);

    for (var i = topoOrder.size() - 1; i >= 0; i--) {
      var n = topoOrder.get(i);

      for (var j : successorIds(n).toArray()) {
        assert topoOrder.indexOf(j) > i; // successor order must be kept in topo order
        result.set(n, j);
        // jian: woc, if it finds that n can reach j, it will set n can reach all the nodes that j can reach
        // here, it adds all the successors of j to the successors set of n.
  ...
  private PolySIMatrixGraph<T> allNodesBfs() {
    var topoOrder = topoSortId().orElse(null);
    if (topoOrder != null) {
      return bfsWithNoCycle(topoOrder);
    }

    var result = new PolySIMatrixGraph<>(this.nodeMap);
  ...
  public PolySIMatrixGraph<T> reachability() {
  ...
  private PolySIMatrixGraph<T> matrixProduct(PolySIMatrixGraph<T> other) {
    assert nodeMap.entrySet().equals(other.nodeMap.entrySet());

    var result = new PolySIMatrixGraph<>(nodeMap);
    for (var i = 0; i < adjacency.length; i++) {
  ...
  public PolySIMatrixGraph<T> composition(PolySIMatrixGraph<T> other) {
    return matrixProduct(other);
  }

  public PolySIMatrixGraph<T> union(PolySIMatrixGraph<T> other) {
    assert nodeMap.entrySet().equals(other.nodeMap.entrySet());
  ...
  public boolean equals(Object obj) {
    if (!(obj instanceof PolySIMatrixGraph)) {
      return false;
    }

    var g = (PolySIMatrixGraph<T>) obj;
  ...
  public int numOfEles(int low, int high) {
    int sum = 0;
    for (var i = low; i <= high; i++){
      sum += adjacency[i].getCardinality();
    }
    return sum;
  }
}
