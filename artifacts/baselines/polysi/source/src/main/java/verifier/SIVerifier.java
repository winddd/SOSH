...
public class SIVerifier<KeyType, ValueType> {
    ...
    public boolean audit() {
        ...
        // Jian: debug
        boolean jianDebug = false;
        if (jianDebug) {
            dumpEdgesCons(graph, constraints, "PolySI_edges_cons_beforepruning.txt");
        }
        ...
        System.err.printf("After Prune:\n" +
                ...
                constraints.stream().map(c -> c.getEdges1().size() + c.getEdges2().size()).reduce(0, Integer::sum));
        if (jianDebug) {
            dumpEdgesCons(graph, constraints, "PolySI_edges_cons_afterpruning.txt");
        }
    ...
    private static <KeyType, ValueType> Collection<SIConstraint<KeyType, ValueType>> generateConstraintsCoalesce(
        ...
        for (var a : history.getTransactions()) {
            for (var b : readFrom.successors(a)) {
                for (var edge : readFrom.edgeValue(a, b).get()) { // traverse all WR edges a-WR->b
                    for (var c : writes.get(edge.getKey())) { // for any write of the same key c
                        if (a == c || b == c) {
                            continue;
                        }
    ...
    private void dumpEdgesCons(KnownGraph<KeyType, ValueType> graph, Collection<SIConstraint<KeyType, ValueType>> constraints, String fileName) {
//        String fileName = ;
        StringBuilder builder = new StringBuilder();
//            builder.append(graph.getKnownGraphA().edges().size()).append("\n")
//                .append("#edges of graphB: ").append(graph.getKnownGraphB().edges().size()).append("\n");
        builder.append("graphA:\n");
        graph.getKnownGraphA().edges().forEach(p -> {
            builder.append(String.format("%s, %s\n", p, graph.getKnownGraphA().edgeValue(p).get()));
        });
        builder.append("graphB:\n");
        graph.getKnownGraphB().edges().forEach(p -> builder.append(String.format("%s, %s\n", p, graph.getKnownGraphB().edgeValue(p).get())));
        builder.append("\n");

        try (BufferedWriter writer = new BufferedWriter(new FileWriter(fileName))) {
            builder.append("#Constraints: ").append(constraints.size()).append("\n");
