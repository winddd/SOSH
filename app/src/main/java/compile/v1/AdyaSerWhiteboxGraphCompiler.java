...
public class AdyaSerWhiteboxGraphCompiler extends GraphCompiler {
  public AdyaSerWhiteboxGraphCompiler(Config cfg) {
    super(cfg);
  }
  ...
  public InCompleteGraph compile(ASG asg) {
    ...
    for (var directDep : asg.getDependencyStore().directDeps()) {
      ...
      if (nextWriteTxn != null) {
        // anti-dependencies
        g.addEdge(new TypeEdge(GraphNodeId.txn(writeTxnId), GraphNodeId.txn(nextWriteTxn.getTxnId()), RW, directDep.key));
      }
    }

    // build write-dependencies by version order
      ...
      for (var key : asg.getKeys()) {
        ...
        if (key.inBetween(rangeMop.getKey1(), rangeMop.getKey2())) { // key belongs to the key range
          if (versionSet.contains(key)) {
            ...
            // TODO: actually PWR and PRW better?
...
//          } else {
//            g.addEdge(new TypeEdge(writeTxnId, rangeTxnId, PWR, key));
//            //
//            KVTxn nextTxn;
//            if (versionSet.contains(key)) {
...
//              g.addEdge(new TypeEdge(rangeTxnId, nextTxn.getTxnId(), PRW, key));
//          }
//        }
      }
    }
