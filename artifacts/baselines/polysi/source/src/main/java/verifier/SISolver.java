...
class SISolver<KeyType, ValueType> {
    ...
    SISolver(History<KeyType, ValueType> history,
        ...
        int lastNumLits = 0;
        System.out.printf("Initially, #Lits: %d, #delta: %d\n", getAllLits().size(), getAllLits().size() - lastNumLits);
        lastNumLits = getAllLits().size();
        ...
        System.err.printf("Known Graph A edges count : %d, Known Graph B edges count: %d\n",
            graphA.edges().size(), graphB.edges().size());
        ...
        System.out.printf("After encoding known edges, #Lits: %d, #delta: %d\n", getAllLits().size(), getAllLits().size() - lastNumLits);
        lastNumLits = getAllLits().size();
        addConstraints(constraints, graphA, graphB);
        System.out.printf("After encoding constraints, #Lits: %d, #delta: %d\n", getAllLits().size(), getAllLits().size() - lastNumLits);
        lastNumLits = getAllLits().size();
        var unknownEdges = Utils.getUnknownEdges(graphA, graphB, reachability,
                solver);
        System.out.printf("After encoding unknown edges, #Lits: %d, #delta: %d\n", getAllLits().size(), getAllLits().size() - lastNumLits);
        lastNumLits = getAllLits().size();
        ...
        System.out.printf("After adding known edges to mono graph, #Lits: %d, #delta: %d\n", getAllLits().size(), getAllLits().size() - lastNumLits);
        lastNumLits = getAllLits().size();
        unknownEdges.forEach(addToMonoSAT);
        System.out.printf("After adding unknown edges to mono graph, #Lits: %d, #delta: %d\n", getAllLits().size(), getAllLits().size() - lastNumLits);
        lastNumLits = getAllLits().size();
    ...
    // create the known graph with Lit, not with edgetypes and keys
    private MutableValueGraph<Transaction<KeyType, ValueType>, Collection<Lit>> createKnownGraph(
        ...
        for (var e : knownGraph.edges()) {
            ...
            // combine multiple edges of different edgetypes and keys into one
            // Jian: will assert all the Lits in knownLiterals to be True
            knownLiterals.put(lit, Pair.of(e, knownGraph.edgeValue(e).get()));
            // jian debug
            assert !g.hasEdgeConnecting(e.source(), e.target());
    ...
    private void addConstraints(
        ...
        var addEdges = ((Function<Collection<SIEdge<KeyType, ValueType>>, Pair<Lit, Lit>>) edges -> {
            ...
            for (var e : edges) {
                ...
                // add unknown edges to graph A and B
