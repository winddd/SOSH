...
public class MatrixGraph<T> implements MutableGraph<T> {
    ...
    public MatrixGraph(Graph<T> graph) {
        ...
        if (topoOrder.isEmpty()) {
            ...
            // assign an index in the topo order to each txn
    ...
    private MatrixGraph<T> matrixProduct(MatrixGraph<T> other) {
        ...
        for (var i = 0; i < adjacency.length; i++) {
            for (var j : adjacency[i]) {
                // jian: actually polysi should compute graphA compost graphB and graphB compose graphA,
                // it is correct to only consider one direction?
                // jian: also composition should not use or operation to keep original elements in adjacency[i]
    ...
    private int numOfEles(int low, int high) {
        int sum = 0;
