...
public class Pruning {
    ...
    static <KeyType, ValueType> boolean pruneConstraints(KnownGraph<KeyType, ValueType> knownGraph,
        ...
        while (!hasCycle) {
            ...
            hasCycle = result.getRight(); // check whether new cycles are introduced after this round of pruning
            solvedConstraints += result.getLeft();

            // if this rounds solves too few constraints, or the remaining constraints are too few.
    ...
    private static <KeyType, ValueType> Pair<Integer, Boolean> pruneConstraintsWithPostChecking(
        ...
        var reachability = Utils.reduceEdges(graphAC, orderInSession).reachability();
