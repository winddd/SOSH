...
history_name="$2"
run_name="$3"
if [[ ! "${figure}" =~ ^fig(10|19)$ ]] || [[ "${history_name}" == */* || "${history_name}" == *..* ]]; then
  echo "ERROR: expected fig10 or fig19 and a direct input-directory name" >&2
  exit 2
fi
...
if [[ ! -d "${host_history}" ]]; then
  echo "ERROR: copied Cobra history does not exist: ${host_history}" >&2
  exit 2
fi
mkdir -p "${results_dir}"
