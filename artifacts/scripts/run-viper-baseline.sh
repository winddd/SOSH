...
if [[ ! -d "${host_history}" ]]; then
  echo "ERROR: copied Viper history does not exist: ${host_history}" >&2
  exit 2
fi
mkdir -p "${results_dir}"
...
echo "Results: ${results_dir}/${run_name}.{jsonl,stdout.log,time.txt}"
...
docker run --rm \
  ...
  --entrypoint bash \
  "${image}" \
  -c 'mkdir -p /tmp/viper/graphs /tmp/viper/analysis && cd /opt/baselines/viper/src && exec /usr/bin/time -v -o "$TIME_LOG" timeout "$TIMEOUT" python3 -u main_allcases.py --config_file /opt/baselines/viper/config.artifact.yaml --algo 6 --sub_dir "$1" --format cobra --strong-session --perf_file "/results/$2.jsonl" --exp_name "$2"' \
  _ "${history_name}" "${run_name}" 2>&1 | tee "${results_dir}/${run_name}.stdout.log"
